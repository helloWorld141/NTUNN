...
#optimizer
df = (y+1)*(3-y)*0.25
grad_w = -tf.matmul(tf.transpose(x), (d - y)*df)
grad_b = -tf.reduce_sum((d - y)*df)
...
for i in range(no_iters):
  ...
  if (i < 2 or i == no_iters-1):
  	u_, y_, df_, loss_, w_, b_ = sess.run([u, y, df, loss, w, b], {x: X, d: Y})
  	print('iter: {}'.format(i+1))
  	print('u: {}'.format(u_))
  	print('y: {}'.format(y_))
  	print('df: {}'.format(df_))
  	print('m.s.e: {}'.format(loss_/7))
  	print('w: {}, b: {}'.format(w_, b_))

  if i%100 == 0 and i != 0:
    print('iter: {}, loss: {}'.format(i, err[i]))
...
plt.xlabel('epochs')
plt.ylabel('cost')
plt.savefig('./figures/2.3a_1.png')
...
plt.axis('equal')
plt.savefig('./figures/2.3a_2.png')
...
ax.set_zlabel('$y$')
ax.legend()
plt.savefig('./figures/2.3a_3.png')
...
# ax.legend()
plt.savefig('./figures/2.3a_4.png')
