...
def createModel(graph=None):
    ...
    w2 = tf.Variable(tf.truncated_normal([num_neurons, num_neurons], stddev=1.0/math.sqrt(float(num_neurons)), seed=25), name='weights2')
    b2  = tf.Variable(tf.zeros([num_neurons]), name='biases2')
    h2  = tf.nn.softmax(tf.matmul(h1, w2) + b2) 
    w3 = tf.Variable(tf.truncated_normal([num_neurons, NUM_CLASSES], stddev=1.0/math.sqrt(float(num_neurons)), seed=25), name='weights3')
    b3  = tf.Variable(tf.zeros([NUM_CLASSES]), name='biases3')
    y  = tf.matmul(h2, w3) + b3  #### linear ouput of NN, prediction values
